...
void show_Menu(SDL_Renderer * renderer, int& x, int& y, bool chooselevel, int level_start){
    ...
    TTF_Font* font = TTF_OpenFont("Font/ChockABlockNF.ttf", size_text);

    if(target_Range(x, y, range_NEWGAME)) color = { 192, 192, 192 };
    else color = { 128, 155, 0 };
    show_Text(renderer, "NEW GAME", color, font, range_NEWGAME, false);

    if(target_Range(x, y, range_CHOOSELEVEL)) color = { 192, 192, 192 };
    else color = { 128, 155, 0 };
    show_Text(renderer, "CHOOSE LEVEL", color, font, range_CHOOSELEVEL, false);

    if(chooselevel){
        TTF_Font* font1 = TTF_OpenFont("Font/VeraMoBd.ttf", 50);

        if(level_start == 1) color = { 192, 192, 192 };
        else color = { 0, 128, 128 };
        show_Text(renderer, "1", color, font1, range_1, false);

        if(level_start == 2) color = { 192, 192, 192 };
        else color = { 0, 128, 128 };
        show_Text(renderer, "2", color, font1, range_2, false);

        if(level_start == 3) color = { 192, 192, 192 };
        else color = { 0, 128, 128 };
    ...
    if(target_Range(x, y, range_HIGHSCORE)){
        color = { 255, 106, 106 };
        TTF_Font* font1 = TTF_OpenFont("Font/spm.otf", 25);
    ...
    if(target_Range(x, y, range_ABOUT)){
        ...
        color = { 128,0,0 };
        TTF_Font* font1 = TTF_OpenFont("Font/bloofs.ttf", 50);
        ...
        SDL_Texture* image = loadTexture("picture/move_1.png", renderer);
        SDL_RenderCopy(renderer, image, NULL, &range);

        range.y = 330;
        image = loadTexture("picture/move_2.png", renderer);
        SDL_RenderCopy(renderer, image, NULL, &range);
        ...
        range.w = 50;
        range.h = 50;
        image = loadTexture("picture/Bullet_Boss21.png", renderer);
        SDL_RenderCopy(renderer, image, NULL, &range);

        range.x = 250;
        image = loadTexture("picture/Bullet_Boss22.png", renderer);
        SDL_RenderCopy(renderer, image, NULL, &range);
        ...
        range.w = 20;
        range.h = 20;
        image = loadTexture("picture/Bullet_Boss1.png", renderer);
        SDL_RenderCopy(renderer, image, NULL, &range);

        range.x = 200;
        range.y = 620;
        image = loadTexture("picture/Obstacle2.png", renderer);
        SDL_RenderCopy(renderer, image, NULL, &range);
        ...
        range.x = 710;
        range.y = 620;
        image = loadTexture("picture/Obstacle0.png", renderer);
        SDL_RenderCopy(renderer, image, NULL, &range);

        range.x = 760;
        image = loadTexture("picture/Obstacle1.png", renderer);
        SDL_RenderCopy(renderer, image, NULL, &range);

        range.x = 810;
        image = loadTexture("picture/Obstacle3.png", renderer);
        SDL_RenderCopy(renderer, image, NULL, &range);
