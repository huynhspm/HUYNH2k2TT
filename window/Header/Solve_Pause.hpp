...
void solve_Pause(SDL_Renderer* renderer, Game& game){
    ...
    range.w = 400; range.h = 300;
    SDL_RenderFillRect(renderer, &range);

    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 0);
    range.x = 140; range.y = 205;
    range.w = 390; range.h = 290;
    SDL_RenderFillRect(renderer, &range);

    SDL_Color  color;
    color = {255, 255, 255};
    TTF_Font* font = TTF_OpenFont("Font/bloofs.ttf", 50);
    ...
    while (true) {
        ...
        if (e.type == SDL_KEYDOWN){
            if (e.key.keysym.scancode == SDL_SCANCODE_RETURN){
                if(Option) game.mode = REPLAY;
                else game.mode = game.last_mode;
                break;
            };

            bool change = false;
            if (e.key.keysym.sym == SDLK_UP || e.key.keysym.sym == SDLK_w)    change = true;
            if (e.key.keysym.sym == SDLK_DOWN || e.key.keysym.sym == SDLK_s)  change = true;
