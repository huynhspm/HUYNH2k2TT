...
void solve_Image_Plane(SDL_Renderer *renderer, Game &game, Object &plane)
{
    ...
    {
        plane.image = loadTexture(PICTURE_FOLDER + "/Explosion.png", renderer);
    }
    else
    {
        string s = PICTURE_FOLDER + "/Plane";
        if (game.mode == PASS_LEVEL)
            s += char(min(3, game.level + 1) + 48);
        else
...
void show_Plane(SDL_Renderer *renderer, Object &plane, Game &game, Object &boss)
{
    if (!plane.visible)
        return;
    if (game.mode == EXPLOSION && game.time_Immortal % 2 == 0)
        return;
    ...
    if (plane.type == ARMOR)
    {
        plane.armor = loadTexture(PICTURE_FOLDER + "/Armor.png", renderer);
