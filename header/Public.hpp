...
const string MUSIC_FOLDER = "../music";
const string PICTURE_FOLDER = "../picture";
const string SCORE_PATH = "../src/score.text";
...
{
    if (font == NULL)
        logSDLError(cout, "TTF OpenFont", true);
...
bool intersect_Rect(Rectangle &rect1, Rectangle &rect2)
{
    if (rect1.x_right > BROAD_WIDTH)
    {
        ...
        if (x_left_Max >= x_right_Min && x_left_MAX >= x_right_MIN)
            return false;
    }
    else
    {
...
{
    if ((range.x_left < x && x < range.x_right) && (range.y_up < y && y < range.y_down))
        return true;
    return false;
}
