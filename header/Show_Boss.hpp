...
void solve_Image_Boss(SDL_Renderer *renderer, Game &game, Object &boss)
{
    ...
    {
        boss.image = loadTexture(PICTURE_FOLDER + "/Explosion.png", renderer);
        return;
    }

    if (game.level <= 2)
    {
        string s = PICTURE_FOLDER + "/Boss";
    ...
    else if (game.level == 3)
    {
        if (game.time % 16 == 9)
        {
            boss.image = loadTexture(PICTURE_FOLDER + "/Boss_Angry.png", renderer);
        }
        else if (game.time % 16 == 0)
        {
            boss.image = loadTexture(PICTURE_FOLDER + "/Boss_Simple.png", renderer);
        }
    }
}
