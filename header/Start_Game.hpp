                ...
                Object &boss, Bullet &bullet_boss, Game &game, Background &background)
{
    ...
    Mix_Music *music_pass_level = Mix_LoadMUS((MUSIC_FOLDER + "/PassLevel.mp3").c_str());
    Mix_Music *music_explosion = Mix_LoadMUS((MUSIC_FOLDER + "/Explosion.mp3").c_str());
    Mix_Music *music_get_item = Mix_LoadMUS((MUSIC_FOLDER + "/GetItem.mp3").c_str());
    Mix_Chunk *music_normal = Mix_LoadWAV((MUSIC_FOLDER + "/Normal.wav").c_str());
    ...
    while (true)
    {
        ...
        if (game.music == START && !Mix_Playing(-1))
            Mix_PlayChannel(-1, music_normal, -1);
        if (game.mode == PAUSE)
            solve_Pause(renderer, game);
